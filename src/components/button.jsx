const Button = ({ text, color, bgcolor_border, onClick }) => {
  return (
    <button
      className={`relative px-5 py-2.5 overflow-hidden font-medium text-gray-600 border ${bgcolor_border}  rounded-lg shadow-inner group mt-6`}
      type="button"
      onClick={onClick}
    >
      ...
      <span
        className={`absolute inset-0 w-full h-full duration-300 delay-300 ${color} opacity-0 group-hover:opacity-100`}
      ></span>
